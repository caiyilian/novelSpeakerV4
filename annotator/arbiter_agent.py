# -*- coding: utf-8 -*-
"""Scheme 2 (docs/discussion/2026-09-21_0932): divergence arbitration layer.

Two independent channels already exist and are verified:
  A = cont_agent  (per-quote rolling conversation, 92.31%)
  B = evledger    (whole-volume evidence + ledger, 91.47%)
They disagree on ~819 of 7009 quotes. This scheme adds the component 0932 calls
its "highest-priority increment": a pure-Python divergence detector plus a
high-reasoning arbiter that sees BOTH candidates + local context + the ledger,
and only then rewrites the divergent entries.

Per 0932 axiom 1 (information increment): the arbiter receives a different
information set (two candidate answers produced from different contexts) than
either channel alone — so this is NOT same-source voting.

Everything outside the divergent set is left untouched (protects the 6075 both-
correct quotes).
"""
...
ARB_SYSTEM = (
    ...
    "【答案形式】说话人用账本里的规范名，或原文里的稳定泛称。仅当这句引语**无法归属到"
    "任何具体人物**时（纯环境声/动物声/被引用的词句/无主体的引用）才填 `无人称引语`；"
    "若上下文有「某人 + 动作/神态/心理/言说」能挂住它（含神态字幕式引号、单独强调的词），"
    "就归该主体。\n"
...
def main():
    ...
    ap.add_argument("--tag", default="arb")
    ap.add_argument("--radius", type=int, default=15)
    ap.add_argument("--gap", type=int, default=5)
    ap.add_argument("--max-clusters", type=int, default=0, help="0=all (smoke test otherwise)")
    args = ap.parse_args()
    ...
    print(f"vol{args.volume}: cont {ok_cont}/{total}={round(ok_cont*100/total,2)}%  →  "
          f"仲裁后 {ok_arb}/{total}={round(ok_arb*100/total,2)}%  "
